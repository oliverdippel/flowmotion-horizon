...
[![Coverage](assets/coverage.svg)](#tests-and-ci)
...
CI (`.github/workflows/ci.yml`) runs ruff (lint + format check), mypy, and the test
suite with coverage on every push and pull request. Coverage is measured over
`src/flowmotion` with `cli.py` excluded (`[tool.coverage.report]` in
`pyproject.toml`) — `cli.py` is argparse wiring around already-tested library
functions, which the test suite exercises directly by calling them rather than by
invoking the CLI (see `tests/test_smoke_e2e.py`), so counting its lines as "untested"
would misrepresent what's actually unverified. On that basis coverage is 95%
(`--cov-fail-under=90` in CI, giving some room before a real regression fails the
build). The badge above is a static snapshot, regenerated by hand rather than by CI,
via:

```bash
uv run coverage run -m pytest -q && uv run coverage-badge -o assets/coverage.svg -f
```
