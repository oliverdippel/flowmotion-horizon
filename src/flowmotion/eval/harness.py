...
class EvalConfig:
    ...
    seed_base: int = 0
    yaw_align: bool = False  # must match how the checkpoint's model was trained


def _trial_seed(
    seed_base: int, subject_key: str, seq_name: str, seed_idx: int, length: int, noise_idx: int
) -> int:
    ...
    key = f"{seed_base}:{subject_key}:{seq_name}:{seed_idx}:{length}:{noise_idx}"
    ...
    for meta, feat in loaded:
        ...
        for seed_idx in range(cfg.seeds_per_subject):
            ...
            for length in cfg.rollout_lengths:
                ...
                for noise_idx in range(cfg.noise_samples):
                    trial_seed = _trial_seed(
                        cfg.seed_base, meta.subject_key, meta.path.stem, seed_idx, length, noise_idx
                    )
